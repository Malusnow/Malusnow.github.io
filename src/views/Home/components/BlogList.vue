...
const posts = getAllPostsMetadata().slice(0, 3) // 获取最新的3篇文章
...
<template>
  ...
  <div class="space-y-4 my-14">
    <article v-for="post in posts" :key="post.id" class="md:grid md:grid-cols-6">
      <div
        class="relative isolate flex flex-col items-start rounded-2xl px-4 py-6 hover:bg-sidebar-ring/30 md:col-span-5 border border-border"
      >
        ...
        <time
          class="relative order-first mb-3 flex items-center pl-3.5 text-sm text-muted-foreground md:hidden"
          :datetime="post.date"
        >
        ...
        <div aria-hidden="true" class="mt-4 flex items-center text-sm font-medium text-chart-2">
          Read article &rarr;
        </div>
      </div>
      <time
        class="order-first hidden text-sm text-muted-foreground md:block"
        :datetime="post.date"
        >{{ post.date }}</time
      >
    </article>
