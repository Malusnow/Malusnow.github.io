...
function parseFrontmatter (raw) {
    ...
    for (const line of frontMatterText.split('\n')) {
        ...
        const value = line.slice(seperator + 1).trim().replace(/^['\"]|['\"]$/g, '');
        if (key) {
            metadata[key] = value;
        }
    }
...
const postList = Object.entries(postModules).map(([path, raw]) => parsePosts(path, raw)).sort((a, b) => new Date(b.date) - new Date(a.date));
